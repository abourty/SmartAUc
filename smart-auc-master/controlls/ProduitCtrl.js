...
let ProduitsCtrl = {
  AjouterProduits: async (req, res) => {
    try {
      ...
      let newProduit = new Produits({
        ...
        description,
        prix,
        dateLimite: expiryDate.toDateString(),
        image: url_imga,
  ...
  getAllProduits: async (req, res) => {
    try {
      ...
      let filterArray = AllProduits.filter(
  ...
  getAllMesProduits: async (req, res) => {
    try {
      ...
      let Arrayfilter = AllProduits.filter(
  ...
  encherProduit: async (req, res) => {
    try {
      ...
      const requiredIndex = findProduit.mombre_enchere.findIndex((el) => {
        return el.idUser.toString() === userConnecte._id.toString();
      });
      if (requiredIndex === -1) {
